...
- [x] AuthManager 클래스 구현
  - [x] `auth-manager.js` 새 파일 생성
  - [x] 로그인/로그아웃 상태 관리
  - [x] 토큰 저장 및 검증
- [x] 로그인 UI 추가
  - [x] Google Sign-In 버튼 추가
  - [x] 사용자 프로필 표시 영역
  - [x] 로그인 상태별 UI 전환
...
### Phase 3-B 완료 기준  
- [x] 로그인/로그아웃 UI가 완전히 작동
- [x] 게임 완료 시 점수가 자동으로 글로벌 리더보드에 업로드
- [x] 리더보드 화면에서 실시간 순위 확인 가능
- [x] Feature Flag로 기능 활성화/비활성화 가능
- [x] 모든 기존 기능이 정상 작동 (하위 호환성)
